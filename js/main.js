...
if ($('.terminal-view')) {
  ...
  function confirmCommand(e) {
    if (e.keyCode === 13) {
      ...
      if (getter.textContent.toLowerCase().trim() === commandsTab[1].toLowerCase().trim()) {
        createPortfolio();
      }
      if (getter.textContent.toLowerCase().trim() === commandsTab[2].toLowerCase().trim()) {
        createContact();
      }
      if (getter.textContent.toLowerCase().trim() === commandsTab[3].toLowerCase().trim()) {
        clearTerminal();
      }
      if (getter.textContent.toLowerCase().trim() === commandsTab[4].toLowerCase().trim()) {
        displayCommands();
      }
      textarea.value = '';
      getter.textContent = '';
